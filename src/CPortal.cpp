...
void CPortal::Init()
{
	...
	for (i=0 ; i<MAX_CLIENTS ; i++) {
		if (eth32.cg.players[i].eth32user && eth32.cg.players[i].playerType == PLAYER_FRIEND)
	...
	if (numUsers) {
		for (i=0 ; i<numUsers ; i++)
		{
			if (!strcmp(eth32.server, users[i].server) && users[i].clientNum >= 0 && users[i].clientNum < MAX_CLIENTS) {
				...
				// portal setting takes presedence
				if (users[i].frnd)
					users[i].player->playerType = PLAYER_FRIEND; 
			}
			else
				users[i].player = NULL;
...
bool CPortal::ParseUserInfo(const char *string, int num)
{
	...
	if (eth32.cgameLoaded && !strcmp(eth32.server, users[num].server) && users[num].clientNum >= 0 && users[num].clientNum < MAX_CLIENTS) {
		...
		if (users[num].frnd)
			users[num].player->playerType = PLAYER_FRIEND; 		
	}
	else
...
void CPortal::RetrieveUsers(void)
{
	...
	for (int k=0 ; k<MAX_CLIENTS ; k++) {
		if (eth32.cg.players[k].eth32user && eth32.cg.players[k].playerType == PLAYER_FRIEND)
			eth32.cg.players[k].playerType == PLAYER_NORMAL;
...
void CPortal::RemoveFriend(const char *user)
{
	...
	for (int i=0 ; i<numUsers ; i++)
	{
		if (!strcmp(user, users[i].username)) {
			...
			if (users[i].player)
				users[i].player->playerType = PLAYER_NORMAL;
